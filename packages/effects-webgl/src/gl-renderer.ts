...
export class GLRenderer extends Renderer implements Disposable {
  ...
  override drawGeometryInstanced (geometry: Geometry, material: Material, instanceCount: number, subMeshIndex = 0): void {
    ...
    // 使用实例化渲染
    const ext = (gl as any).drawArraysInstanced ? gl : (gl as WebGLRenderingContext).getExtension('ANGLE_instanced_arrays');
    ...
    if (indicesBuffer) {
      if ((gl as any).drawElementsInstanced) {
        (gl as WebGL2RenderingContext).drawElementsInstanced(mode, count, indicesBuffer.type, offset ?? 0, instanceCount);
      } else {
        (ext as ANGLE_instanced_arrays).drawElementsInstancedANGLE(mode, count, indicesBuffer.type, offset ?? 0, instanceCount);
      }
    } else {
      if ((gl as any).drawArraysInstanced) {
        (gl as WebGL2RenderingContext).drawArraysInstanced(mode, offset, count, instanceCount);
      } else {
        (ext as ANGLE_instanced_arrays).drawArraysInstancedANGLE(mode, offset, count, instanceCount);
      }
    }
    vao?.unbind();
  }

...
export class ModelMeshComponent extends RendererComponent {
  ...
  override fromData (data: ModelMeshComponentData): void {
    ...
    this.data.rootBone = data.rootBone ? this.findObject(data.rootBone) : undefined;
    this.data.geometry = this.findObject(data.geometry);

    if (data.materials) {
      for (let i = 0; i < data.materials.length; i++) {
        this.data.materials[i] = this.findObject(data.materials[i]);
      }
    }
  }
...
export class ModelSkyboxComponent extends RendererComponent {
  ...
  override fromData (data: ModelSkyboxComponentData): void {
    ...
    data.diffuseImage = data.diffuseImage ? this.findObject(data.diffuseImage) : undefined;
    data.specularImage = this.findObject(data.specularImage);
    this.data = data;
  }
...
export class AnimationComponent extends Behaviour {
  ...
  override fromData (data: AnimationComponentData): void {
    ...
    data.animationClips.forEach(clipData => {
      ...
      const animationClip = this.findObject<AnimationClip>(clipData);

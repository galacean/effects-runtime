...
import { StringSet, ArrayUtils } from '../utils';
...
export abstract class Timeline {
  ...
  constructor (frameCount: number, propertyIds: string[]) {
    ...
    this.frames = ArrayUtils.newFloatArray(frameCount * this.getFrameEntries());
  }
  ...
  static search1 (frames: NumberArrayLike | Array<number>, time: number) {
  ...
  static search (frames: NumberArrayLike | Array<number>, time: number, step: number) {
    const n = frames.length;
...
export abstract class CurveTimeline extends Timeline {
  protected curves: NumberArrayLike | Array<number>; // type, x, y, ...

  constructor (frameCount: number, bezierCount: number, propertyIds: string[]) {
    super(frameCount, propertyIds);
    this.curves = ArrayUtils.newFloatArray(frameCount + bezierCount * 18/*BEZIER_SIZE*/);
  ...
  shrink (bezierCount: number) {
    ...
    if (this.curves.length > size) {
      const newCurves = ArrayUtils.newFloatArray(size);

      ArrayUtils.arrayCopy(this.curves, 0, newCurves, 0, size);
...
export class DeformTimeline extends CurveTimeline implements SlotTimeline {
  ...
  constructor (frameCount: number, bezierCount: number, slotIndex: number, attachment: VertexAttachment) {
    ...
    this.vertices = new Array<NumberArrayLike | Array<number>>(frameCount);
  }
  ...
  setFrame (frame: number, time: number, vertices: NumberArrayLike | Array<number>) {
    this.frames[frame] = time;
  ...
  override setBezier (bezier: number, frame: number, value: number, time1: number, value1: number, cx1: number, cy1: number, cx2: number,
    cy2: number, time2: number, value2: number) {
    const curves = this.curves;
    let i = this.getFrameCount() + bezier * 18/*BEZIER_SIZE*/;
  ...
  apply (skeleton: Skeleton, lastTime: number, time: number, firedEvents: Array<Event>, alpha: number, blend: MixBlend, direction: MixDirection) {
    ...
    if (time >= frames[frames.length - 1]) { // Time is after last frame.
      ...
      if (alpha == 1) {
        ...
        } else { ArrayUtils.arrayCopy(lastVertices, 0, deform, 0, vertexCount); }
      } else {
        switch (blend) {
          case MixBlend.setup: {
            const vertexAttachment = slotAttachment;
...
export class DrawOrderTimeline extends Timeline {
  ...
  apply (skeleton: Skeleton, lastTime: number, time: number, firedEvents: Array<Event>, alpha: number, blend: MixBlend, direction: MixDirection) {
    if (direction == MixDirection.mixOut) {
      if (blend == MixBlend.setup) { ArrayUtils.arrayCopy(skeleton.slots, 0, skeleton.drawOrder, 0, skeleton.slots.length); }

      return;
    }

    if (time < this.frames[0]) {
      if (blend == MixBlend.setup || blend == MixBlend.first) { ArrayUtils.arrayCopy(skeleton.slots, 0, skeleton.drawOrder, 0, skeleton.slots.length); }
    ...
    if (!drawOrderToSetupIndex) { ArrayUtils.arrayCopy(skeleton.slots, 0, skeleton.drawOrder, 0, skeleton.slots.length); } else {

...
import { StringSet, Pool, ArrayUtils } from '../utils';
...
export class AnimationState {
    ...
    for (let i = 0, n = tracks.length; i < n; i++) {
      ...
      if ((i == 0 && mix == 1) || blend == MixBlend.add) {
        for (let ii = 0; ii < timelineCount; ii++) {
          ...
          ArrayUtils.webkit602BugfixHelper(mix, blend);
    ...
    if (blend == MixBlend.add) {
      ...
      for (let i = 0; i < timelineCount; i++) {
        ...
        if (!shortestRotation && timeline instanceof RotateTimeline) {this.applyRotateTimeline(timeline, skeleton, applyTime, alpha, timelineBlend, from.timelinesRotation, i << 1, firstFrame);} else if (timeline instanceof AttachmentTimeline) {this.applyAttachmentTimeline(timeline, skeleton, applyTime, timelineBlend, attachments);} else {
          // This fixes the WebKit 602 specific issue described at http://esotericsoftware.com/forum/iOS-10-disappearing-graphics-10109
          ArrayUtils.webkit602BugfixHelper(alpha, blend);
  ...
  expandToIndex (index: number) {
    ...
    ArrayUtils.ensureArrayCapacity(this.tracks, index + 1, null);
  ...
  computeHold (entry: TrackEntry) {
    ...
    for (let i = 0; i < timelinesCount; i++) {
      ...
      if (!propertyIDs.addAll(ids)) {timelineMode[i] = SUBSEQUENT;} else if (!to || timeline instanceof AttachmentTimeline || timeline instanceof DrawOrderTimeline
...
export class TrackEntry {
  ...
  /** Seconds for mixing from the previous animation to this animation. Defaults to the value provided by AnimationStateData
	 ...
	 * afterward. */
  mixDuration = 0; interruptAlpha = 0; totalAlpha = 0;

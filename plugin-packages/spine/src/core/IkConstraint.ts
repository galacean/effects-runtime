...
export class IkConstraint implements Updatable {
  ...
  apply2 (parent: Bone, child: Bone, targetX: number, targetY: number, bendDir: number, stretch: boolean, uniform: boolean, softness: number, alpha: number) {
    const px = parent.ax, py = parent.ay;
    let psx = parent.ascaleX, psy = parent.ascaleY, sx = psx, sy = psy, csx = child.ascaleX;
    ...
    const cx = child.ax;
    let cy = 0, cwx = 0, cwy = 0, a = parent.a, b = parent.b, c = parent.c, d = parent.d;
    ...
    d = pp.d;
    const id = 1 / (a * d - b * c);
    let x = cwx - pp.worldX, y = cwy - pp.worldY;
    const dx = (x * d - y * b) * id - px, dy = (y * a - x * c) * id - py;
    const l1 = Math.sqrt(dx * dx + dy * dy);
    let l2 = child.data.length * csx, a1, a2;

...
import { Pool, ArrayUtils } from '../utils';
...
export class SkeletonBounds {
  ...
  polygons = new Array<NumberArrayLike | Array<number>>();

  private polygonPool = new Pool<NumberArrayLike | Array<number>>(() => {
    return ArrayUtils.newFloatArray(16);
  });
    ...
    for (let i = 0; i < slotCount; i++) {
      ...
      if (attachment instanceof BoundingBoxAttachment) {
        ...
        if (polygon.length != boundingBox.worldVerticesLength) {
          polygon = ArrayUtils.newFloatArray(boundingBox.worldVerticesLength);
        }
  ...
  containsPointPolygon (polygon: NumberArrayLike | Array<number>, x: number, y: number) {
    const vertices = polygon;
  ...
  intersectsSegmentPolygon (polygon: NumberArrayLike | Array<number>, x1: number, y1: number, x2: number, y2: number) {
    const vertices = polygon;
    const nn = polygon.length;

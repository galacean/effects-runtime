...
export class SkeletonClipping {
  ...
  clipStart (slot: Slot, clip: ClippingAttachment): number {
    ...
    const vertices = ArrayUtils.setArraySize(this.clippingPolygon, n);
  ...
  clipTriangles (vertices: Array<number>, verticesLength: number, triangles: Array<number>, trianglesLength: number, uvs: NumberArrayLike | Array<number>,
    ...
    for (let i = 0; i < trianglesLength; i += 3) {
      ...
      for (let p = 0; p < polygonsCount; p++) {
        ...
        if (this.clip(x1, y1, x2, y2, x3, y3, polygons[p], clipOutput)) {
          ...
          const clippedVerticesItems = ArrayUtils.setArraySize(clippedVertices, s + clipOutputCount * vertexSize);
          ...
          const clippedTrianglesItems = ArrayUtils.setArraySize(clippedTriangles, s + 3 * (clipOutputCount - 2));
          ...
          const clippedVerticesItems = ArrayUtils.setArraySize(clippedVertices, s + 3 * vertexSize);
          ...
          s = clippedTriangles.length;
          const clippedTrianglesItems = ArrayUtils.setArraySize(clippedTriangles, s + 3);
  ...
  public static makeClockwise (polygon: NumberArrayLike | Array<number>) {

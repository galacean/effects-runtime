import type { Timeline, CurveTimeline1, CurveTimeline2, CurveTimeline } from './Animation';
...
import type { VertexAttachment, Attachment } from './attachments';
import type { AttachmentLoader } from './attachments';
import type { MeshAttachment } from './attachments';
...
import { ArrayUtils, Color } from '../utils';
...
export class SkeletonJson {
  ...
  readSkeletonData (json: string): SkeletonData {
    ...
    if (root.bones) {
      for (let i = 0; i < root.bones.length; i++) {
        ...
        data.transformMode = ArrayUtils.enumValue(TransformMode, getValue(boneMap, 'transform', 'Normal'));
    ...
    if (root.slots) {
      for (let i = 0; i < root.slots.length; i++) {
        ...
        data.blendMode = ArrayUtils.enumValue(BlendMode, getValue(slotMap, 'blend', 'normal'));
    ...
    if (root.path) {
      for (let i = 0; i < root.path.length; i++) {
        ...
        data.positionMode = ArrayUtils.enumValue(PositionMode, getValue(constraintMap, 'positionMode', 'Percent'));
        data.spacingMode = ArrayUtils.enumValue(SpacingMode, getValue(constraintMap, 'spacingMode', 'Length'));
        data.rotateMode = ArrayUtils.enumValue(RotateMode, getValue(constraintMap, 'rotateMode', 'Tangent'));
        data.offsetRotation = getValue(constraintMap, 'rotation', 0);
  ...
  readAttachment (map: any, skin: Skin, slotIndex: number, name: string, skeletonData: SkeletonData): Attachment | null {
    ...
    switch (getValue(map, 'type', 'region')) {
      ...
      case 'path': {
        ...
        const lengths: Array<number> = ArrayUtils.newArray(vertexCount / 3, 0);
  ...
  readVertices (map: any, attachment: VertexAttachment, verticesLength: number) {
    ...
    if (verticesLength == vertices.length) {
      const scaledVertices = ArrayUtils.toFloatArray(vertices);
    ...
    attachment.vertices = ArrayUtils.toFloatArray(weights);
  }
  ...
  readAnimation (map: any, name: string, skeletonData: SkeletonData) {
          ...
          for (const attachmentMapName in slotMap) {
            ...
            for (const timelineMapName in attachmentMap) {
              ...
              if (timelineMapName == 'deform') {
                ...
                for (let frame = 0, bezier = 0; ; frame++) {
                  let deform: NumberArrayLike | Array<number>;
                  const verticesValue: Array<Number> = getValue(keyMap, 'vertices', null);

                  if (!verticesValue) {deform = weighted ? ArrayUtils.newFloatArray(deformLength) : vertices;} else {
                    deform = ArrayUtils.newFloatArray(deformLength);
                    const start = <number>getValue(keyMap, 'offset', 0);

                    ArrayUtils.arrayCopy(verticesValue, 0, deform, start, verticesValue.length);
                    if (scale != 1) {
    ...
    if (map.drawOrder) {
      ...
      for (let i = 0; i < map.drawOrder.length; i++, frame++) {
        ...
        if (offsets) {
          drawOrder = ArrayUtils.newArray<number>(slotCount, -1);
          const unchanged = ArrayUtils.newArray<number>(slotCount - offsets.length, 0);
    ...
    if (map.events) {
      ...
      for (let i = 0; i < map.events.length; i++, frame++) {
        ...
        const event = new Event(ArrayUtils.toSinglePrecision(getValue(eventMap, 'time', 0)), eventData);

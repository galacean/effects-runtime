import { AssetManager, Player, math } from '@galacean/effects';
...
(async () => {
  try {
    ...
    const jsons = [json, json, json, json, json];
    // const comps = await player.loadScene(jsons);
    // for (let i = 0;i < comps.length;i++) {
    //   const comp = comps[i];

    //   comp.setPosition(0, 5 * i, 0);
    // }
    await Promise.all(jsons.map(json=>{
      return player.loadScene(json);
    })

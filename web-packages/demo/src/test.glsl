...
void main() {
    ...
    float boundaryTransition = smoothstep(-transitionRange, transitionRange, signedDist);
    ...
    if(signedDist < 0.0) {
        ...
        if (lineStroke == 0.0) {
            // 在边界处alpha从1.0渐变到0.0
            finalAlpha = smoothstep(0.0, -transitionRange, signedDist);
        } else {
            finalAlpha = 1.0;
        }
    ...
    if (signedDist > 0.0) {
        ...
        if (lineStroke == 0.0) {
            // 混合内侧颜色和辉光
            finalColorRGB = mix(_InsideColor, glowBlend, boundaryTransition);
            // 透明度从0.0渐变到辉光强度
            finalAlpha = mix(0.0, glow * _GlowIntensity, boundaryTransition);
        } else {
            // 在线条区域保持辉光
            finalColorRGB = glowBlend;
            finalAlpha = max(lineStroke, glow * _GlowIntensity);
        }
    }

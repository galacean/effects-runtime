...
export class MemoryEditor {
  ...
  CalcSizes (s: MemoryEditor.Sizes, mem_size: size_t, base_display_addr: size_t): void {
    ...
    if (s.AddrDigitsCount === 0) {
      for (let /*size_t*/ n = base_display_addr + mem_size - 1; n > 0; n >>= 4) {s.AddrDigitsCount++;}
    }
    ...
    if (this.OptShowAscii) {
      ...
      if (this.OptMidColsCount > 0) {s.PosAsciiStart += /*(float)*/((this.Cols + this.OptMidColsCount - 1) / this.OptMidColsCount) * s.SpacingBetweenMidCols;}

...
export class Hierarchy extends EditorWindow {
  ...
  override onGUI () {
    ...
    if (this.searchFilter.length > 0) {
      ...
      if (compositionNodeOpen) {
        this.drawVFXItemTreeNode(composition.sceneRoot, baseFlags);
        ImGui.TreePop();
      }
    }
  ...
  private updateSearchMatches (): void {
    ...
    // 递归收集所有匹配项（平铺列表）
    this.collectMatchedItems(composition.sceneRoot, filterLower);
  }
  ...
  private buildFlatItemList (composition: Composition): void {
    ...
    // 递归收集所有 VFXItem
    this.collectItemsFlat(composition.sceneRoot);
  }

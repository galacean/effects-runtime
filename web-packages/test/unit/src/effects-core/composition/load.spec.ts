...
describe('load error', () => {
  ...
  it('loadScene with different text variables', async () => {
    const [comp1, comp2] = await player.loadScene([
      {
        scene: 'https://mdn.alipayobjects.com/mars/afts/file/A*_QkURKxu0TEAAAAAAAAAAAAADlB4AQ',
        options: {
          autoplay: true,
          variables: {
            text_908: 'ttt',
    ...
    const t1 = comp1.getItemByName('text_908');
    const t2 = comp2.getItemByName('text_908');

    expect(t1.content.text).to.eql('ttt');
    expect(t2.content.text).to.eql('xxx');
  });
});

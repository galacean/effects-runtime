...
describe('webgl/geometry', () => {
  ...
  it('updates only updatable buffers', () => {
    ...
    readBufferContents((engine.renderingDevice as RenderingDeviceWebGL).gl, staticBuffer.getBuffer()!, staticResult);
    readBufferContents((engine.renderingDevice as RenderingDeviceWebGL).gl, dynamicBuffer.getBuffer()!, dynamicResult);
  ...
  it('uses float units for direct update offsets and releases partial CPU data', () => {
    ...
    readBufferContents((engine.renderingDevice as RenderingDeviceWebGL).gl, buffer.getBuffer()!, result);
  ...
  it('uploads full and partial changes through the shared buffer', () => {
    ...
    readBufferContents((engine.renderingDevice as RenderingDeviceWebGL).gl, dataBuffer, result);
  ...
  it('uploads only the changed index range', () => {
    ...
    readBufferContents((engine.renderingDevice as RenderingDeviceWebGL).gl, indexBuffer, result, 0, true);
  ...
  it('replaces the index buffer when its element type changes', () => {
    ...
    readBufferContents((engine.renderingDevice as RenderingDeviceWebGL).gl, geometry.getIndexBuffer()!, result, 0, true);
  ...
  it('disposes cached vertex array objects when the layout changes', () => {
    const geometry = createGeometry(engine);
    const resource = (engine.renderingDevice as RenderingDeviceWebGL).gl.createVertexArray()!;
    const releaseVertexArrayObject = (engine.renderingDevice as RenderingDeviceWebGL).releaseVertexArrayObject.bind(engine.renderingDevice);
    let released = false;

    (engine.renderingDevice as RenderingDeviceWebGL).releaseVertexArrayObject = vertexArrayObject => {
      released = vertexArrayObject === resource;
      releaseVertexArrayObject(vertexArrayObject);
    };
    (engine.renderingDevice as RenderingDeviceWebGL).recordVertexArrayObject = () => resource;
  ...
  it('discards cached vertex array objects without releasing them during restore', () => {
    ...
    geometry.initialize();
    (engine.renderingDevice as RenderingDeviceWebGL).releaseVertexArrayObject = () => {
      released = true;
    };
    (engine.renderingDevice as RenderingDeviceWebGL).recordVertexArrayObject = () => {
      recorded++;
  ...
  it('keeps indexed drawStart in bytes', () => {
    const gl = (engine.renderingDevice as RenderingDeviceWebGL).gl;

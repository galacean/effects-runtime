...
function emulateContextLoss (engine: Engine): Promise<void> {
  ...
  const ext = ((engine.renderingDevice as RenderingDeviceWebGL).gl as WebGLRenderingContext).getExtension('WEBGL_lose_context');
...
function canEmulateContextLoss (engine: Engine): boolean {
  return !!((engine.renderingDevice as RenderingDeviceWebGL).gl as WebGLRenderingContext).getExtension('WEBGL_lose_context');
}
...
describe('webgl/gl-context-lost', () => {
  ...
  describe('opt-in 自动恢复（doNotHandleContextLost=false）', () => {
    beforeEach(() => {
      ...
      gl = (engine.renderingDevice as RenderingDeviceWebGL).gl as WebGLRenderingContext;
    ...
    it('shader program 在 restore 后被重建', async function () {
      ...
      const library = (engine.renderingDevice as RenderingDeviceWebGL).shaderLibrary;
    ...
    it('几何缓冲区在 restore 后被重建', async function () {
      ...
      readBufferContents((engine.renderingDevice as RenderingDeviceWebGL).gl, vertexBuffer.getBuffer()!, vertices);
      readBufferContents((engine.renderingDevice as RenderingDeviceWebGL).gl, geometry.getIndexBuffer()!, indices, 0, true);
    ...
    it('粒子系统在 geometry restore 后重建动态缓冲内容', async function () {
      ...
      readBufferContents((engine.renderingDevice as RenderingDeviceWebGL).gl, geometry.getVertexBuffer('aPosition')!.getBuffer()!, restored);

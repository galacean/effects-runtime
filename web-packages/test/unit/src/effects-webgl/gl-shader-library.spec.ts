...
describe('webgl/gl-shader-library', () => {
  ...
  before(() => {
    ...
    const glEngine1 = new Engine(webglCanvas, { glType: 'webgl' });
    const glEngine2 = new Engine(webgl2Canvas, { glType: 'webgl2' });
  ...
  it('create material shader from shaderlib', async () => {
    const shaderLib = (rendererGL2.engine.renderingDevice as RenderingDeviceWebGL).shaderLibrary;
    ...
    (rendererGL2.engine.renderingDevice as RenderingDeviceWebGL).shaderLibrary.compileShader(shader);
  ...
  // TODO 目前删除方式改变，待补充。
  // const shaderLib = (renderer.engine.renderingDevice as RenderingDeviceWebGL).shaderLibrary;
  ...
  // (renderer.engine.renderingDevice as RenderingDeviceWebGL).shaderLibrary.compileShader(shaderId);
  ...
  it('compile shader async', function (done) {
    ...
    const shaderLib = (engine.renderingDevice as RenderingDeviceWebGL).shaderLibrary;
    const shader = shaderLib.createShader({
      vertex: vs,
      fragment: fs,
  ...
  it('compile shader async work if no extension', function (done) {
    const glEngine = rendererGL2.engine.renderingDevice as RenderingDeviceWebGL;

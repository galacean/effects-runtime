...
describe('webgl/gl-state', () => {
  ...
  it('WebGL default value', () => {
    ...
    assert.equal((state.graphicsServer.renderingDevice as RenderingDeviceWebGL).get('gl.DITHER'), null);
  });
  ...
  it('WebGL framebuffer and depth function test ', () => {
    ...
    (state.graphicsServer.renderingDevice as RenderingDeviceWebGL).bindFramebuffer(gl.FRAMEBUFFER, framebuffer);
    assert.equal(gl.getParameter(gl.FRAMEBUFFER_BINDING), framebuffer, 'fbo');
    state.graphicsServer.renderingDevice.bindSystemFramebuffer();
    assert.equal(gl.getParameter(gl.FRAMEBUFFER_BINDING), null, 'fbo null');
    //end framebuffer
    //clear
    gl.clearColor(1, 1, 1, 1);
    (state.graphicsServer.renderingDevice as RenderingDeviceWebGL).gl.clear(gl.COLOR_BUFFER_BIT);
    expect(gl.getParameter(gl.COLOR_CLEAR_VALUE)).deep.equals(new Float32Array([1, 1, 1, 1]));
    //clear depth
    (state.graphicsServer.renderingDevice as RenderingDeviceWebGL).clearDepth(1);
    assert.equal(gl.getParameter(gl.DEPTH_CLEAR_VALUE), 1, 'DEPTH_CLEAR_VALUE');
    //depth func
    (state.graphicsServer.renderingDevice as RenderingDeviceWebGL).enable(gl.DEPTH_TEST);
    state.graphicsServer.renderingDevice.depthFunc(gl.NEVER);
    assert.equal(gl.getParameter(gl.DEPTH_FUNC), gl.NEVER, 'DEPTH_FUNC');
    //depth mash
    state.graphicsServer.renderingDevice.depthMask(false);
    assert.equal(gl.getParameter(gl.DEPTH_WRITEMASK), false);

    //depth range
    state.graphicsServer.renderingDevice.depthRange(0.2, 0.6);
  ...
  it('stencil function test', () => {
    ...
    //stencil start
    (state.graphicsServer.renderingDevice as RenderingDeviceWebGL).clearStencil(1.0);
    assert.equal(gl.getParameter(gl.STENCIL_CLEAR_VALUE), 1.0);
    //stencli mask
    (state.graphicsServer.renderingDevice as RenderingDeviceWebGL).stencilMask(110101);
    ...
    //stencilfunc
    (state.graphicsServer.renderingDevice as RenderingDeviceWebGL).stencilFunc(gl.LESS, 0.1, 0b1110011);
    assert.equal(gl.getParameter(gl.STENCIL_FUNC), gl.LESS);
    //stencilFuncSeparate
    state.graphicsServer.renderingDevice.stencilFuncSeparate(gl.BACK, gl.NEVER, 0.2, 1110011);
    ...
    //stencilMaskSeparate
    state.graphicsServer.renderingDevice.stencilMaskSeparate(gl.FRONT, 111001);
    assert.equal(gl.getParameter(gl.STENCIL_WRITEMASK), 111001);
    //stencilOp
    (state.graphicsServer.renderingDevice as RenderingDeviceWebGL).stencilOp(gl.KEEP, gl.KEEP, gl.KEEP);
    ...
    //stencilOpSeparate
    state.graphicsServer.renderingDevice.stencilOpSeparate(gl.BACK, gl.KEEP, gl.DECR_WRAP, gl.KEEP);
    ...
    //cull face
    state.graphicsServer.renderingDevice.cullFace(gl.BACK);
    assert.equal(gl.getParameter(gl.CULL_FACE_MODE), gl.BACK);
    //frontFace
    state.graphicsServer.renderingDevice.frontFace(gl.CCW);
  });

  it('color function test', () => {
    const state = engine;

    (state.graphicsServer.renderingDevice as RenderingDeviceWebGL).clearColor(1.0, 1.0, 0.8, 1.0);
    expect(gl.getParameter(gl.COLOR_CLEAR_VALUE)).deep.equal(new Float32Array([1.0, 1.0, 0.8, 1.0]));

    //color mask
    state.graphicsServer.renderingDevice.colorMask(true, true, false, false);
  ...
  it('blend function test', () => {
    ...
    //blend color
    state.graphicsServer.renderingDevice.blendColor(0, 0.5, 1, 1);
    expect(gl.getParameter(gl.BLEND_COLOR)).deep.equal(new Float32Array([0, 0.5, 1, 1]));

    //blendFunc
    (state.graphicsServer.renderingDevice as RenderingDeviceWebGL).blendFunc(gl.SRC_COLOR, gl.DST_COLOR);
    assert.equal(gl.getParameter(gl.BLEND_SRC_RGB), gl.SRC_COLOR);
    //blendFuncSeparate
    state.graphicsServer.renderingDevice.blendFuncSeparate(gl.SRC_COLOR, gl.DST_COLOR, gl.ONE, gl.ZERO);
    assert.equal(gl.getParameter(gl.BLEND_SRC_RGB), gl.SRC_COLOR);
    //blendEquation
    (state.graphicsServer.renderingDevice as RenderingDeviceWebGL).blendEquation(gl.FUNC_ADD);
    assert.equal(gl.getParameter(gl.BLEND_EQUATION_RGB), gl.FUNC_ADD);
    //blendEquationSeparate
    state.graphicsServer.renderingDevice.blendEquationSeparate(gl.FUNC_REVERSE_SUBTRACT, gl.FUNC_SUBTRACT);
  ...
  it('texture adn others function test', () => {
    ...
    (state.graphicsServer.renderingDevice as RenderingDeviceWebGL).bindTexture(gl.TEXTURE_2D, texture);
    assert.equal(gl.getParameter(gl.TEXTURE_BINDING_2D), texture);
    //gl.pixelStorei(gl.PACK_ALIGNMENT, 4);
    (state.graphicsServer.renderingDevice as RenderingDeviceWebGL).setPixelStorei(gl.PACK_ALIGNMENT, 4);
    assert.equal(gl.getParameter(gl.PACK_ALIGNMENT), 4);
    //viewport
    state.graphicsServer.renderingDevice.setViewport(0, 0, 900, 800);

...
describe('plugin-gui/GUI topology', () => {
  ...
  it('keeps the window as the only RootControl and canvases as ordinary containers', () => {
    ...
    expect(player.engine.getService(GUIService)).instanceOf(GUIService);
    expect(player.engine.getService(GUIService)!.windowRoot).instanceOf(RootControl);
    expect(canvas.rootControl).instanceOf(Control);
    expect(canvas.rootControl).not.instanceOf(RootControl);
    expect(player.engine.getService(GUIService)!.windowRoot.canvases.parent).equals(player.engine.getService(GUIService)!.windowRoot);
    ...
    expect(canvas.rootControl.parent).equals(player.engine.getService(GUIService)!.windowRoot.canvases);
    expect(canvas.rootControl.root).equals(player.engine.getService(GUIService)!.windowRoot);
  });
  ...
  it('isolates the GUI root and window state between engines', () => {
    ...
    const firstWindow = player.engine.getService(GUIService)!;
    const secondWindow = other.engine.getService(GUIService)!;

    expect(secondWindow).not.equals(firstWindow);
    expect(secondWindow.windowRoot).not.equals(firstWindow.windowRoot);
    expect(other.engine.getService(GUIService)).equals(secondWindow);
  ...
  it('initializes GUI before engine-created plugins can create scenes', () => {
    let created = false;

    class SceneCreator extends Plugin {
      override onEngineCreated (engine: Player['engine']): void {
        const gui = engine.getService(GUIService)!;
        const scene = new Composition(engine);

        scene.root.initializeHierarchy();
        scene.root.beginPlay();
        expect(scene.sceneRoot.getComponent(UICanvas).rootControl.parent).equals(gui.windowRoot.canvases);
        created = true;
      }

      override onEngineDestroy (engine: Player['engine']): void {
        expect(engine.compositions).length(1);
        expect(engine.getService(GUIService)!.windowRoot.isDisposed).equals(false);
      }
    }
    registerPlugin('test-gui-composition-order', SceneCreator);
    const other = new Player({ canvas: document.createElement('canvas'), manualRender: true });
  ...
  it('detaches scene canvases before disposing GUI and removes runtime subscriptions', () => {
    ...
    gui.windowRoot.dispose = () => {
      expect(canvas.isDisposed).equals(true);
      expect(gui.windowRoot.canvases.children).not.includes(canvas);
      calls.push('dispose');
      dispose();
    };
    engine.dispose();
    ...
    gui.windowRoot.render = () => calls.push('render');
    gui.windowRoot.resize = () => calls.push('resize');
  ...
  it('creates an isolated default canvas for every composition', () => {
    ...
    expect(first.root.parent).equals(undefined);
    expect(first.root.composition).equals(first);
    expect(second.root.parent).equals(undefined);
    expect(second.root.composition).equals(second);
    ...
    expect(engine.getService(GUIService)!.windowRoot.canvases.children).includes(first.sceneRoot.getComponent(UICanvas).rootControl);
    expect(engine.getService(GUIService)!.windowRoot.canvases.children).includes(second.sceneRoot.getComponent(UICanvas).rootControl);
    ...
    engine.getService(GUIService)!.windowRoot.canvases.sortCanvases();
    ...
    expect(engine.getService(GUIService)!.windowRoot.canvases.children.indexOf(second.sceneRoot.getComponent(UICanvas).rootControl))
      .lessThan(engine.getService(GUIService)!.windowRoot.canvases.children.indexOf(first.sceneRoot.getComponent(UICanvas).rootControl));
    ...
    expect(engine.getService(GUIService)!.windowRoot.canvases.children).not.includes(firstRoot);
    expect(engine.getService(GUIService)!.windowRoot.canvases.children).not.includes(secondRoot);
  ...
  it('builds a GUI tree through UIControl bridges', () => {
    ...
    expect(parentControl.root).equals(player.engine.getService(GUIService)!.windowRoot);
    expect(childControl.root).equals(player.engine.getService(GUIService)!.windowRoot);
  });
  ...
  it('sorts independent canvases by UICanvas.order', () => {
    ...
    player.engine.getService(GUIService)!.windowRoot.canvases.sortCanvases();
    expect(player.engine.getService(GUIService)!.windowRoot.canvases.children[0]).equals(overlay.rootControl);
    expect(player.engine.getService(GUIService)!.windowRoot.canvases.children[1]).equals(composition.sceneRoot.getComponent(UICanvas).rootControl);
  ...
  it('attaches and detaches a canvas through enable state', () => {
    ...
    expect(composition.sceneRoot.getComponent(UICanvas).rootControl.parent).equals(player.engine.getService(GUIService)!.windowRoot.canvases);
  });
  ...
  it('synchronizes a canvas to the current window size when attaching late', () => {
    ...
    composition.sceneRoot.getComponent(UICanvas).enabled = false;
    player.engine.getService(GUIService)!.windowRoot.resize(640, 360);
  ...
  it('notifies the old Root only once when a Control is disposed', () => {
    ...
    chai.spy.on(player.engine.getService(GUIService)!.windowRoot, 'controlRemoved');
    control.dispose();
    expect(player.engine.getService(GUIService)!.windowRoot.controlRemoved).to.have.been.called.once;
  });
});

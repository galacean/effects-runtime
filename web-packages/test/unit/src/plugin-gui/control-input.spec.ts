...
describe('plugin-gui/input', () => {
  ...
  it('dispatches to the front-most control and bubbles through Pass parents', () => {
    ...
    player.engine.getService(GUIService)!.windowRoot.pushInput(event);
  ...
  it('uses InputEvent acceptance to stop bubbling and report handled input', () => {
    ...
    expect(event.isAccepted()).equals(true);
    expect(player.engine.getService(GUIService)!.windowRoot.isInputHandled()).equals(true);
  ...
  it('releases mouse focus on canvas blur while preserving other GUI state', () => {
    const windowRoot = player.engine.getService(GUIService)!.windowRoot;
    const control = addControl(
      composition.sceneRoot,
  ...
  it('keeps EventSystem disposal silent and lets GUIService clean up independently', () => {
    const guiWindow = player.engine.getService(GUIService)!;
  ...
  it('uses reverse child order for hit testing', () => {
    ...
    player.engine.getService(GUIService)!.windowRoot.pushInput(mouseButton(10, 10, true));
  ...
  it('routes hit testing through nested local transforms', () => {
    ...
    parent.setScale(2, 2);
    player.engine.getService(GUIService)!.windowRoot.pushInput(mouseButton(25, 15, true));
  ...
  it('defers dropping a hidden hover target without searching for a sibling', () => {
    ...
    motion.globalPosition.copyFrom(motion.position);
    player.engine.getService(GUIService)!.windowRoot.pushInput(motion);

    expect(front.log).deep.equals(['enter']);
    expect(back.log).not.includes('enter');

    player.engine.getService(GUIService)!.windowRoot.update(0);
  ...
  it('routes keyboard input to the focused control', () => {
    ...
    key.pressed = true;
    key.keycode = 'Enter';
    player.engine.getService(GUIService)!.windowRoot.pushInput(key);
  ...
  it('keeps one focused control across all canvases in a window', () => {
    ...
    expect(first.root).equals(player.engine.getService(GUIService)!.windowRoot);
    expect(second.root).equals(player.engine.getService(GUIService)!.windowRoot);
    expect(player.engine.getService(GUIService)!.windowRoot.guiGetFocusOwner()).equals(second);

    player.engine.getService(GUIService)!.windowRoot.guiReleaseFocus();
    expect(second.log).deep.equals(['focus', 'blur']);
    expect(player.engine.getService(GUIService)!.windowRoot.guiGetFocusOwner()).equals(null);
  });
  ...
  it('keeps touch capture in the window GUIState across canvases', () => {
    ...
    player.engine.getService(GUIService)!.windowRoot.pushInput(touch);
    const drag = new InputEventScreenDrag();

    drag.index = 7;
    drag.position.set(110, 10);
    drag.relative.set(100, 0);
    player.engine.getService(GUIService)!.windowRoot.pushInput(drag);
  ...
  it('preserves GUIState when a Control moves within the same Root', () => {
    ...
    expect(player.engine.getService(GUIService)!.windowRoot.guiGetFocusOwner()).equals(child);
  ...
  it('stores mouse position on the window and derives local control coordinates', () => {
    ...
    expect(player.engine.getService(GUIService)!.windowRoot.getMousePosition()).deep.equals(new Vector2(35, 55));

...
describe('plugin-gui/GUI clipping and scrolling', () => {
  ...
  it('passes wheel input to an outer container at an inner boundary', () => {
    ...
    wheel.factor = 1;
    player.engine.getService(GUIService)!.windowRoot.pushInput(wheel);
  ...
  it('drags with a deadzone and continues with inertial scrolling', () => {
    ...
    player.engine.getService(GUIService)!.windowRoot.pushInput(down);
    ...
    player.engine.getService(GUIService)!.windowRoot.pushInput(up);
    scroll.update(100);
  ...
  it('emits guiFocusChanged from the root when the focus owner changes', () => {
    ...
    player.engine.getService(GUIService)!.windowRoot.on('guiFocusChanged', focusChanged);
    control.grabFocus();
    control.grabFocus();
    control.releaseFocus();
    player.engine.getService(GUIService)!.windowRoot.off('guiFocusChanged', focusChanged);
    control.grabFocus();
